use std::{io::{self, Seek, Write, Read}, cell::RefCell};

use bincode::{Encode, Decode, config::{LittleEndian, Fixint, Configuration, SkipFixedArrayLength, NoLimit}};
...
const BINCODE_CONFIG: Configuration<LittleEndian, Fixint, SkipFixedArrayLength, NoLimit> = bincode::config::legacy()
    .skip_fixed_array_length().with_fixed_int_encoding().with_little_endian();
...
impl<'a, W: Encode + Sized, S: DataSourceSource> WriteData<W> for DataSource<S> {
    ...
    fn write_at(&self, data: &W, position: u64) -> Result<u64, DataSourceError> {
        let serialized = self.serialize(data).map_err(|_| DataSourceError::WriteDataError)?;
...
impl<'a, R: Decode + Sized, S: DataSourceSource> ReadData<R> for DataSource<S> {
    fn deserialize(&self, data: &[u8]) -> Result<R, DataSourceError> {
        bincode::decode_from_slice(&data, BINCODE_CONFIG).map_err(|_| DataSourceError::DeserializeError).map(|(result, _)| result)
    }

    fn read_at(&self, position: u64) -> Result<DataPointer<R>, DataSourceError> {
        //println!("DEBUG: {}", position);
        self.source.borrow_mut().seek(io::SeekFrom::Start(position)).map_err(|_| DataSourceError::ReadDataError)?;
        bincode::decode_from_std_read(&mut *self.source.borrow_mut(), BINCODE_CONFIG).map_err(|_| DataSourceError::ReadDataError).map(|r| DataPointer { data: r, position })
    }
}
...
impl<'a, S: DataSourceSource> ReadBytes for DataSource<S> {
    fn read_bytes_at(&self, position: u64, size: u64) -> Result<Vec<u8>, DataSourceError> {
        ...
        self.source.borrow_mut().read_exact(buf.as_mut_slice()).map_err(|_| DataSourceError::ReadDataError).map(|_| buf)
    }
}

impl<'a, S: DataSourceSource> WriteBytes for DataSource<S> {
    fn write_bytes_at(&self, data: &[u8], position: u64) -> Result<u64, DataSourceError> {
        self.source.borrow_mut().seek(io::SeekFrom::Start(position)).map_err(|_| DataSourceError::WriteDataError)?;
        self.source.borrow_mut().write_all(data).map_err(|_| DataSourceError::WriteDataError).map(|_| data.len() as u64)
    }
}

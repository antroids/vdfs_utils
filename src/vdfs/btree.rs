...
use super::{layout::*, layout::consts::*, data_source::*};
...
impl<'a, 'b: 'a, S: DataSourceSource> VdfsBtree<'a, S> {
    pub fn new(data_source: &'b DataSource<S>, 
                super_blocks: Vdfs4SuperBlocks,
                base_table: DataPointer<Vdfs4BaseTable>, 
                btree_type: BtreeType) -> Result<VdfsBtree<'a, S>, BtreeError> {
        let log_blocks_in_leb = super_blocks.super_block.log_super_page_size - super_blocks.super_block.log_block_size;
        ...
        let head_bnode: DataPointer<Vdfs4HeadBtreeNode> = Self::read_base_table_record(&btree, data_source, &base_table, 0)?;
    ...
    fn read_base_table_record<T: VdfsBtreeNode>(&self, 
                data_source: &DataSource<impl DataSourceSource>, 
                base_table: &DataPointer<Vdfs4BaseTable>, 
                node_id: u32) -> Result<DataPointer<T>, BtreeError> {
        ...
        if table_record.get_version() != iblock_descriptor.data.get_version() {
            return Err(BtreeError::NodeAndBaseTableVersionsMismatch(iblock_descriptor.data.get_version(), table_record.get_version()));
        }
    ...
    fn get_bnode_offset_position(&self, bnode: &DataPointer<Vdfs4GeneralBtreeNode>, index: u16) -> u64 {
        bnode.position + self.get_bnode_offset_offset(index)
    }

    fn get_bnode_offset_from_data_source(&self, bnode: &DataPointer<Vdfs4GeneralBtreeNode>, index: u16) -> Result<u32, BtreeError> {
    ...
    fn get_bnode_offset_from_buffer(&self, buffer: &[u8], bnode: &DataPointer<Vdfs4GeneralBtreeNode>, index: u16) -> Result<u32, BtreeError> {
        if index > bnode.data.get_last_record_index() {
            return Err(BtreeError::BnodeRecordIndexOutOfBounds(index))
        }

        let bnode_offset_offset = self.get_bnode_offset_offset(index);
        let bnode_offset: u32 = self.data_source.deserialize(&buffer[bnode_offset_offset as usize..])?;
        if bnode_offset == 0 || bnode_offset as u64 >= self.node_size_bytes {
            return Err(BtreeError::BnodeRecordOffsetOutOfBounds(bnode_offset))
        }
    ...
    fn get_bnode_record<T: VdfsBtreeKey>(&self, bnode: &DataPointer<Vdfs4GeneralBtreeNode>, index: u16) -> Result<DataPointer<T>, BtreeError> {
        let bnode_offset: u32 = self.get_bnode_offset_from_data_source(bnode, index)?;
        let bnode_record: DataPointer<T> = self.data_source.read_at(bnode.position + bnode_offset as u64)?;
        Ok(bnode_record)
    }

    fn get_bnode_record_from_buffer<T: VdfsBtreeKey>(&self, buffer: &[u8], bnode: &DataPointer<Vdfs4GeneralBtreeNode>, index: u16) -> Result<DataPointer<T>, BtreeError> {
        let bnode_offset: u32 = self.get_bnode_offset_from_buffer(buffer, bnode, index)?;
        let bnode_record: T = self.data_source.deserialize(&buffer[bnode_offset as usize..])?;
        Ok(DataPointer { data: bnode_record, position: bnode.position + bnode_offset as u64 })
    }

    fn traverse<T: VdfsBtreeKey>(&self, key: &T, till_level: u16) -> Result<BnodeRecordInfo<T>, BtreeError> {
        ...
        for level in ((till_level + 1)..=max_tree_level).rev() {
            let index_value: GenericIndexValue = bnode_record_info.key.get_record_value(self.data_source)?.data;
    ...
    fn traverse_level<T: VdfsBtreeKey>(&self, key: &T, start_bnode_id: u32) -> Result<BnodeRecordInfo<T>, BtreeError> {
        let mut bnode: DataPointer<Vdfs4GeneralBtreeNode> = self.read_base_table_record(self.data_source, &self.base_table, start_bnode_id)?;
        let mut bnode_buffer = self.data_source.read_bytes_at(bnode.position, self.node_size_bytes)?;
        let (mut bnode_record_index, mut bnode_record): (u16, DataPointer<T>) = self.binary_search_in_bnode(&bnode_buffer, key, &bnode)?;

        // Dangling nodes are not handled
        while bnode_record_index == bnode.data.get_last_record_index() && bnode.data.next_node_id != 0 {
            bnode = self.read_base_table_record(self.data_source, &self.base_table, bnode.data.next_node_id)?;
            bnode_buffer = self.data_source.read_bytes_at(bnode.position, self.node_size_bytes)?;
            (bnode_record_index, bnode_record) = self.binary_search_in_bnode(&bnode_buffer, key, &bnode)?;
        }

        Ok(BnodeRecordInfo { key: bnode_record, bnode: bnode, record_index: bnode_record_index })
    }

    fn binary_search_in_bnode<T: VdfsBtreeKey>(&self, 
            bnode_buffer: &[u8],
            key: &T,
            bnode: &DataPointer<Vdfs4GeneralBtreeNode>) -> Result<(u16, DataPointer<T>), BtreeError> {
        let mut left_index: u16 = 0;
        let mut right_index = bnode.data.recs_count - 1;

        let mut left_record: DataPointer<T> = self.get_bnode_record_from_buffer(&bnode_buffer, bnode, left_index)?;
    ...
    pub fn records_iter<T: VdfsBtreeKey>(&self, start_bnode_id: u32, start_index: u16) -> Result<BtreeRecordsIterator<S, T>, BtreeError> {
        ...
        Ok(BtreeRecordsIterator {
            btree: self,
            bnode_record_info: BnodeRecordInfo { key: first_record, bnode: bnode, record_index: start_index },
            initial_state: true
        })
    }
}
...
impl<'a, S: DataSourceSource, T: VdfsBtreeKey> Iterator for BtreeRecordsIterator<'a, S, T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
            ...
            match self.bnode_record_info.record_index.cmp(&last_record_index) {
                ...
                std::cmp::Ordering::Equal => {
                    ...
                    if next_node_id != VDFS4_INVALID_NODE_ID as u32 {
                        let next_bnode: DataPointer<Vdfs4GeneralBtreeNode> = self.btree.get_bnode(next_node_id).unwrap();
        ...
        return Some(self.btree.get_bnode_record(&self.bnode_record_info.bnode, self.bnode_record_info.record_index).unwrap());
    }
}

impl<T: VdfsBtreeKey> DataPointer<T> {
    pub fn get_record_value<R: Decode + Sized, S: DataSourceSource>(&self, data_source: &DataSource<S>) -> Result<DataPointer<R>, BtreeError> {
        ...
        data_source.read_at(position).map_err(|e| BtreeError::DataSourceError(e))
    }
}

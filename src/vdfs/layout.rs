...
pub trait VdfsBtreeKey: PartialEq + PartialOrd + Encode + Decode {
...
pub trait VdfsBtreeNode: HasSignature + Encode + Decode + HasVersion {
...
impl Vdfs4BaseTable {
	pub fn get_translated_position(&self, base_table_offset: u64, base_table: &Vdfs4BaseTable, table_type: impl TranslationTableIndex) -> u64 {
...
impl PartialOrd for Vdfs4CatTreeKey {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        ...
        match self.name[..(self.name_len as usize)].partial_cmp(&other.name[..(other.name_len as usize)]) {
            Some(core::cmp::Ordering::Equal) => {}
            ord => return ord,
        }
...
impl Vdfs4GenericKey {
	pub fn new() -> Self {
		Self { magic: [0u8; 4], key_len: 0, record_len: 0 }
	}
}
...
impl Vdfs4CatTreeKey {
	...
	pub fn child_of_root() -> Self {
		Self { gen_key: Vdfs4GenericKey::new(), parent_id: SpecialInodeIds::Root as u64, object_id: 0, record_type: 0, name_len: 0, name: [0u8; VDFS4_FILE_NAME_LEN] }
	}
}

impl Vdfs4ExtTreeKey {
	pub fn from_object_id(object_id: u64) -> Self {
		Self { gen_key: Vdfs4GenericKey::new(), object_id, iblock: 0 }
	}
}
